...
- ✅ 올바른 예: `--due "2024-12-20"`, `--due "2024.12.20"`, `--due "2024/12/20"` (시간은 제목이나 설명에 포함)

### 🆕 상대 날짜 지원 (v0.1.5+)
- **상대 날짜 형식 지원**: `+n` (n일 후), `-n` (n일 전)
- ✅ 예시: `--due "+3"` (3일 후), `--due "-1"` (어제), `--due "+0"` (오늘)
- **마감일 표시 개선**: 기한이 지난 할일은 "n일 전" 형태로 표시
...
# 4. 상대 날짜로 할일 추가 (v0.1.5 새로운 기능!)
tasky add "내일 프레젠테이션" --due "+1"
tasky add "3일 후 고객 미팅" --due "+3"
tasky add "어제 놓친 일정 정리" --due "-1"
tasky add "오늘 할 급한 일" --due "+0"
```
...
```bash
# 상대 날짜로 할일 추가
tasky add "1주일 후 프로젝트 마감" --due "+7"
tasky add "2일 전 놓친 리뷰" --due "-2"
tasky add "오늘 마감인 긴급 업무" --due "+0"

# 다양한 상대 날짜 패턴
tasky add "내일까지 코드 리뷰" --due "+1" -p high
tasky add "5일 후 클라이언트 미팅" --due "+5" -p medium
tasky add "지난주 놓친 문서 정리" --due "-7" -p low
```

### 터미널 크기 자동 적응 (v0.1.5+)
```bash
# 긴 제목도 터미널 크기에 맞춰 자동으로 줄여서 표시
tasky add "매우 긴 제목의 할일입니다 이것은 터미널 크기에 따라 자동으로 줄여져서 표시됩니다" -p high

# 터미널 크기를 조정하면 자동으로 제목 표시 길이도 조정됨
tasky list

# 작은 터미널에서도 깨지지 않고 정상 표시
tasky list --today
```

### 개선된 마감일 표시
```bash
# 기한이 지난 할일들은 "n일 전" 형태로 표시됨
tasky list --overdue
# 예시 출력: "3일 전", "1일 전", "7일 전" 등

# 앞으로 다가올 마감일은 "n일 후" 형태로 표시
tasky list --sort due --order asc
# 예시 출력: "1일 후", "3일 후", "1주일 후" 등
```
...
# 🆕 상대 날짜 활용 (v0.1.5+) - 더 간편하게!
tasky add "내일 스프린트 계획 미팅 (10:00)" -p high --due "+1"
tasky add "일일 스탠드업 (09:30)" -p medium --due "+0"  # 오늘
tasky add "주간 회고 미팅 (금요일)" -p medium --due "+2"
...
# 🆕 긴 제목도 자동으로 터미널에 맞춰 표시됨 (v0.1.5+)
tasky add "매우 상세한 프로젝트 요구사항 분석 및 기술 스택 선정을 위한 팀 회의 준비 작업"
# 위 제목도 터미널 크기에 맞춰 "매우 상세한 프로젝트 요구사항 분석 및 기..." 형태로 표시
```
